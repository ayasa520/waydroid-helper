...
from .control_msg import InjectKeycodeMsg, InjectTextMsg
from .input import AKeyEventAction, AMetaState
...
class KeyboardSdk(KeyboardBase):
    # 所有模式都用
    special_keys = {
    ...
    def convert_keycode(self, controller, keyval, keycode, state):
        ...
        if self.inject_mode == KeyInjectMode.RAW:
            ...
            keyval = self.get_low_level_keyval(
    ...
    def get_low_level_keyval(self, display, keycode):
    ...
    def get_reapeat(self, keyval, action):
    ...
    def __key_processor(
        ...
        action = self.convert_action(controller.get_current_event())
        key_code = self.convert_keycode(controller, keyval, keycode, state)
        # metastate = self.convert_mod(state)
        metastate = self.convert_mod(
            controller.get_current_event_device().get_modifier_state()
        )
        ...
        msg = InjectKeycodeMsg(
            action, key_code, self.get_reapeat(keyval, action), metastate
        )
        self.controller.send_msg(msg)
        return True

    def __text_processor(self, controller, keyval, keycode, state):
        event_type = controller.get_current_event().get_event_type()
        if (
            not controller.get_current_event().is_modifier()
        ) and event_type == Gdk.EventType.KEY_PRESS:
